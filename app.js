...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (authForm) {
    ...
    const setMode = mode => { const signingUp = mode === 'signup'; page.dataset.mode = mode; document.querySelector('#mode-label').textContent = signingUp ? 'CREATE YOUR ACCESS' : 'WELCOME BACK'; document.querySelector('#auth-title').innerHTML = signingUp ? 'MAKE IT<br><em>OFFICIAL.</em>' : 'ENTER THE<br><em>LOOP.</em>'; document.querySelector('#auth-subtitle').textContent = signingUp ? 'Claim your spot in the world before the doors open.' : 'New to Opus? Your world is one click away.'; document.querySelector('#auth-button-text').textContent = signingUp ? 'CREATE MY ACCESS' : 'ENTER OPUS'; authForm.password.autocomplete = signingUp ? 'new-password' : 'current-password'; switcher.innerHTML = signingUp ? 'Already a member? <button type="button" data-mode="signin">SIGN IN</button>' : 'Not a member? <button type="button" data-mode="signup">CREATE YOUR ACCESS</button>'; };
